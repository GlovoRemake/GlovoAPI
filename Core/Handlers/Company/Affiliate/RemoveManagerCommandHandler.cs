...
public sealed class RemoveManagerCommandHandler
    : IRequestHandler<RemoveManagerCommand, Result>
{
    private readonly IAffiliateService _affiliateService;

    public RemoveManagerCommandHandler(IAffiliateService affiliateService)
    {
        _affiliateService = affiliateService;
    }

    public async Task<Result> Handle(
        RemoveManagerCommand request,
        CancellationToken cancellationToken)
    {
        try
        {
            await _affiliateService.RemoveManager(request.affiliateId, request.partnerDto);
