    ...
    ) : IEmailService
{
    ...
    public async Task SendForgotPasswordVerificationCodeAsync(string toEmail, string code)
    {
        var subject = "Код підтвердження";

        string fromEmail = _config["Email:User"] ?? "noreply@yourdomain.com";

        string template = await _emailTempService.GetTemplateAsync("forgot-code-verification.html");
